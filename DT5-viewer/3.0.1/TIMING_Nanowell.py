...
class TIMING_Nanowell(QFrame):
    def __init__(self, dataPath, BID, Well_ID, t, t_span, tag):
        ...
        self.CH0_sample = self.path + "B" + str(self.BID).zfill(3) + "/temp/CH0_sample.jpg"

        self.feature_loaded = 0  # indicating whether the feat data is loaded
        self.Display_Flags = {'CH0': True, 'CH1': True, 'CH2': True, 'CH3': False, 'CH1_label': False,
                              'CH2_label': False, 'CH1_box': True, 'CH2_box': True}
        self.timer = QTimer()
        ...
        # update row and column based on block image
    ...
    def getImagePath(self, CH):
        path = self.path + "B" + str(self.BID).zfill(3) + "/images/crops_vis/imgNo" + str(self.Well_ID) + "CH" + str(
            CH) + "/imgNo" + str(self.Well_ID) + "CH" + str(CH) + "_t" + str(self.t) + ".tif"
        # print path
        return path

    def getBGImagePath(self, CH):
        path = self.path + "B" + str(self.BID).zfill(3) + "/crops_8bit_s/imgNo" + str(self.Well_ID) + "CH" + str(
            CH) + "bg/imgNo" + str(self.Well_ID) + "CH" + str(CH) + "bg_t" + str(self.t) + ".tif"
        # print path
        return path

    def getLabelImagePath(self, CH):
        path = self.path + "B" + str(self.BID).zfill(3) + "/label_img/imgNo" + str(self.Well_ID) + "CH" + str(
            CH) + "bg/imgNo" + str(self.Well_ID) + "CH" + str(CH) + "bg_t" + str(self.t) + ".tif"
        return path
    ...
    def getBoxPath(self, cell_type):
        if cell_type == 1:
            path = self.path + "B" + str(self.BID).zfill(3) + "/labels/TRACK/EZ/FRCNN-Fast/imgNo" + str(
                self.Well_ID) + "/label_E_t" + str(self.t).zfill(3) + ".txt"
            # print(path)
            if os.path.isfile(path):
                return path
            else:
        ...
        if cell_type == 2:
            path = self.path + "B" + str(self.BID).zfill(3) + "/labels/TRACK/EZ/FRCNN-Fast/imgNo" + str(
                self.Well_ID) + "/label_T_t" + str(self.t).zfill(3) + ".txt"
            # print(path)
    ...
    def starButtonEvent(self):
        ...
        self.monoWell.append(
            TIMING_Feature_Board(self.path, self.BID, self.Well_ID, self.E, self.T, self.Tag, 1, self.t_span))
    ...
    def update_row_column(self):
        """
		    This function uses template matching to match the nanowell template image to the CHO_sample block image in
		     order to update the row and column of the nanowell.
		    """

        well_img = io.imread(self.getImagePath(0))
        block_img = io.imread(self.CH0_sample)[:, :, 0]

        # match the template and find xmin and ymin of nanowell w.r.t the block image
        matched_template = match_template(block_img, well_img)
        ij = np.unravel_index(np.argmax(matched_template), matched_template.shape)

        # get xmin, ymin, width and height of the template (nanowell)
        xTemplate, yTemplate = ij[::-1]
        wTemplate, hTemplate = well_img.shape

        # find the center of the nanowell and normalize to width/height for column/row
        self.Tag.C = int(np.ceil(float((xTemplate + wTemplate / 2) * 6) / block_img.shape[0]))
        self.Tag.R = int(np.ceil(float((yTemplate + hTemplate / 2) * 6) / block_img.shape[1]))
