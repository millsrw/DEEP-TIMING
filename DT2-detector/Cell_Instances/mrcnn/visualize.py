...
def display_instances_XL(file_path2, image_name,image, boxes, masks, class_ids,CID, class_names,
                      scores=None, uncertainties = None,title="",
                      figsize=(16, 16), ax=None,
                      show_mask=True,show_bbox=True,
                      colors=None, captions=None,
                      mask_erosion_px = None, mask_linewidth =0.3):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    masks: [height, width, num_instances]
    class_ids: [num_instances]
    ...
    for i in range(N):
        ...
        # Label
        if not captions:
            class_id    = CID[i] #RWM class_ids
            score       = scores[i]        if scores        is not None else None
            uncertainty = uncertainties[i] if uncertainties is not None else None
            label       = class_names[class_id] #RWM class_ids
            x = random.randint(x1, (x1 + x2) // 2)
            caption = "{} {:.3f}".format(label, score) if score else label
            if uncertainty is not None :
                caption = "{:.2f}$\pm${:.2f}".format(score,uncertainty) if score else label
    ...
    if auto_show:
        plt.show()
    plt.savefig("{}/{}.png".format(file_path2, image_name))
    print('ok')
