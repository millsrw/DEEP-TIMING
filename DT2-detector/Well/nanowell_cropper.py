...
def get_fname_in(RAW_INPUT_PATH, OUTPUT_PATH, DATASET, BLOCK, CH, t):
    ...
    fname = OUTPUT_PATH + DATASET + '/' + BLOCK + '/temp/preprocess/' + DATASET + '_' + BID + 't' + str(t).zfill(2) + CH + '.tif'
...
def DT_CROP_IMAGES(RAW_INPUT_PATH, OUTPUT_PATH, DATASET, BLOCKS, FRAMES, Output_Type, channel_index_dict, CORES, Nanowell_Size, Block_Size, CLIP_ARGS, GAMMA):
    for BLOCK in BLOCKS:
        print("CROPPING BLOCK " + BLOCK + " ......")
        CROP_IMAGES_BLOCK(RAW_INPUT_PATH, OUTPUT_PATH, DATASET, BLOCK, FRAMES, Output_Type, channel_index_dict, CORES, Nanowell_Size, Block_Size, CLIP_ARGS, GAMMA)
